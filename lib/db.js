...
// Helper to determine writeable path (supporting read-only filesystems by falling back to system /tmp)
function getWritableFilePath(filename) {
  ...
  try {
    ...
    try {
      ...
      console.error("Could not write to /tmp either:", err.message);
      return path.join(localDir, filename);
    }
  }
}
...
export async function getInquiriesData() {
  if (supabase) {
    try {
      ...
      console.warn("Supabase inquiries fetch failed, falling back to local file:", error?.message);
    } catch (err) {
      console.error("Supabase inquiries error:", err.message);
    }
  }

  // Fallback to local inquiries.json
  const filePath = getWritableFilePath("inquiries.json");
  if (!fs.existsSync(filePath)) {
    return [];
  }
  try {
    ...
    console.error("Error reading local inquiries.json", e);
    return [];
  }
}
...
export async function saveInquiry(inquiry) {
  ...
  // 1. Write to local file for backup/local mode
  ...
  try {
    ...
    if (fs.existsSync(filePath)) {
      try {
        ...
        console.error("Error reading inquiries file for append", e);
      }
    }
    currentList.push(newInquiry);
    fs.writeFileSync(filePath, JSON.stringify(currentList, null, 2), "utf-8");
  } catch (err) {
    console.warn("Local inquiries file write failed (expected on serverless read-only filesystems):", err.message);
  ...
  if (supabase) {
    try {
      ...
      console.error("Supabase inquiries insert error:", err.message);
    }
  }

  // 3. Fallback: if local write failed and Supabase also failed/unavailable, throw error
  if (localWriteFailed && !supabaseWriteSuccess) {
    throw new Error(
      "Failed to save inquiry. Local filesystem is read-only and database connection is unavailable: " +
      (localWriteError ? localWriteError.message : "Unknown error")
    );
  }
...
export async function deleteInquiry(id) {
  // 1. Delete from local inquiries.json
  const filePath = getWritableFilePath("inquiries.json");
  if (fs.existsSync(filePath)) {
    try {
      const list = JSON.parse(fs.readFileSync(filePath, "utf-8"));
      ...
      console.error("Error writing inquiries file for delete", e);
    }
  }
  ...
  if (supabase) {
    try {
      ...
      if (error) {
        console.error("Supabase inquiries delete failed:", error.message);
      }
    } catch (err) {
      console.error("Supabase inquiries delete error:", err.message);
    }
  }
